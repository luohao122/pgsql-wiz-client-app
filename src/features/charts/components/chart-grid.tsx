...
import { type IChartDataView, type IChartInfo } from "../interfaces/chart.interface";
        ...
        <div className="h-full overflow-y-auto w-full">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {charts.map((chart: IChartInfo) => (
              <div
                key={chart.id}
                className="bg-white px-3 py-2 rounded-lg border border-gray-200 w-full"
              >
                <div className="flex gap-2 mb-1">
                  <div
                    className="text-ellipsis truncate w-full flex gap-1 items-center"
                    title={chart.chartName}
                  >
                    <i className={chartIcon(chart)} />
                    <div className="flex items-center justify-between gap-3 w-full">
                      <h2 className="font-semibold text-sm text-ellipsis truncate max-w-64">
                        {chart.chartName}
                      </h2>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-1 mb-6">
                  <div
                    className="text-sm text-gray-400 max-w-32 truncate"
                    title={chart.projectId}
                  >
                    {chart.projectId}
                  </div>
                  <span className="text-gray-400 mb-1">|</span>
                  <span className="text-[12px] text-gray-400 text-ellipsis truncate">
